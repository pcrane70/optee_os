...
int get_bnxt_images_info(struct bnxt_images_info *bnxt_info, int chip_type)
{
	...
	vaddr_t flash_dev_vaddr = (uintptr_t)((vaddr_t)(phys_to_virt((paddr_t)
					       QSPI_BNXT_IMG,
					       MEM_AREA_IO_NSEC)));

	bnxt_info->bnxt_bspd_cfg_vaddr = (uintptr_t)((vaddr_t)(phys_to_virt(
						     (paddr_t)QSPI_BSPD_ADDR,
						      MEM_AREA_IO_NSEC)));
	...
	if (chip_type == BCM_NS3) {
		bnxt_info->bnxt_cfg_vaddr = flash_dev_vaddr +
						  fw_image_offset;
